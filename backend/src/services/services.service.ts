...
export class ServicesService implements IServicesService {
  ...
  async create(data: CreateServicesDto) {
    try {
      ...
      this._logger.log(`Услуга с ID ${service.id} успешно создана`);
      return service;
    } catch (error) {
      if (
  ...
  async findAll() {
    try {
      ...
      this._logger.log('Успешно получен список всех услуг');
      return services;
    } catch (error) {
      console.log(error);
      this._logger.error(`Ошибка при поиске всех услуг`, error.stack);
  ...
  async findOne(id: string) {
    try {
      ...
      if (!service) {
        this._logger.warn(`Услуга с ID ${id} не найдена`);
        throw new HttpException('Услуга не найдена', 404);
      }

      this._logger.log(`Услуга с ID ${id} успешно найдена`);
      return service;
    } catch (error) {
      console.log(error);
      this._logger.error(`Ошибка при поиске услуги с ID ${id}`, error.stack);
      throw new HttpException('Ошибка сервера при поиске', 500);
    }
  }
  ...
  async update(id: string, data: UpdateServicesDto) {
    try {
      ...
      this._logger.log(`Услуга с ID ${id} успешно обновлена \n${data}`);
  ...
  async remove(id: string) {
    try {
      ...
      this._logger.log(`Услуга с ID ${id} успешно удалена`);
      return result;
    } catch (error) {
      if (error instanceof PrismaClientKnownRequestError) {
        this._logger.warn(`Услуга с ID ${id} не найдена при удалении`);
        throw new HttpException('Услуга не найдена', 404);
      }
      console.log(error);
      this._logger.error(`Ошибка при удалении услуги с ID ${id}`, error.stack);
