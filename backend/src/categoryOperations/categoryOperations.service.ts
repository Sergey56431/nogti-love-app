...
export class CategoryOperationsService implements ICategoryOperationsService {
  ...
  async create(data: CreateCategoryOperationsDto) {
    try {
      ...
      if (!data.userId) {
        this.logger.warn(`Нет ID пользователя для создания категории операций, ${data}`);
        errors.push(
          new HttpException('Нет ID пользователя', 400).getResponse(),
        );
      }
      if (!data.name) {
        this.logger.warn(`Нет названия категорий операций для создания категории операций, ${data}`);
        errors.push(
          new HttpException(
      ...
      this.logger.log(`Категория операции ${createdCategoryOperation} успешно создана для пользователя с ID ${data.userId}`);
      ...
      if (
        ...
        this.logger.warn(`Пользователь ${data.userId} не найден при создании категории ${data}`);
        throw new HttpException('Пользователь не найден', 404);
      }
      console.log(error);
      this.logger.error(`Пользователь ${data.userId} ввел неверные данные для создания категории операции`, error.stack);
  ...
  async update(id: string, data: UpdateCategoryOperationsDto) {
    try {
      ...
      this.logger.log(`Категория операции с ID ${id} успешно обновлена, ${data}`);
      return updatedCategoryOperation;
    } catch (error) {
      if (error instanceof PrismaClientKnownRequestError && error.code == 'P2025') {
        this.logger.warn(`Категория операции с ID ${id} не найдена при обновлении ${data}`);
        throw new HttpException('Категория операции не найдена', 404);
      }
      console.log(error);
      this.logger.error(`Пользователь ${data.userId} ввел неверные данные для обновления категории операции ${id}`, error.stack);
